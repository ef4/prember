...
class Prerender extends Plugin {
  ...
  async build() {
    ...
    */
    ...
    const allUrls = await this.listUrls(this.protocol, this.host);

    const urlChunks = chunk(allUrls, this.requestsPerFastboot);
    let hadFailures = false;

    for (let urls of urlChunks) {
      try {
        await this.buildChunk(urls);
      } catch (err) {
        hadFailures = true;
      }
    }
  ...
  async buildChunk(urls) {
    ...
    if (hadFailures) {
      throw new Error('Chunk had some errors');
    }
  }
...
function chunk(array, n) {
  return range(Math.ceil(array.length / n)).map((_, i) =>
